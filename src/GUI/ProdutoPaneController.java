...
private PanelaFit panelaFit;
	...
	public void alterarProduto() throws ProdutoNaoExisteException, ProdutoJaExisteException, FormatacaoInvalidaException, ValidationException, IOException{
		if(validateFields()){
			try{
				String nome;
				Integer codigo = new Integer(txtCodigoProduto.getText());
	...
	private void validateAttributes(Produto produto) throws ValidationException{
		...
		if(produto.getDataValidade()==null){
			...
			if(!returnMs.isEmpty()){
				throw new ValidationException(String.format("Os arumentos[%s] obrigatorios estao nulos ou com valores invalidos", returnMs));
			}
	...
	private boolean validateFields() throws IOException{
		...
		try{
			if((txtNomeProduto.getText().isEmpty() || !txtNomeProduto.getText().matches("[a-z A-Z]+")) || 
					(txtCodigoProduto.getText().isEmpty() || !txtCodigoProduto.getText().matches("[0-9][0-9][0-9][0-9][0-9]"))||
						(txtPesoProduto.getText().isEmpty() || !txtPesoProduto.getText().matches("[0-9][0-9][0-9]G")) || 
							(txtCaloriasProduto.getText().isEmpty() || !txtCaloriasProduto.getText().matches("[0-9]+")) ||
								(txtPrecoProduto.getText().isEmpty() || !txtPrecoProduto.getText().matches("[R][$][0-9][0-9][0-9]")) || 
									(txtQuantEstoqueProduto.getText().isEmpty() || !txtQuantEstoqueProduto.getText().matches("[0-9]+")) || 
										(txtDataFab.getText().isEmpty() || !txtDataFab.getText().matches("[0-9][0-9][/][0-9][0-9][/][0-9][0-9][0-9][0-9]")) ||
											(txtDataVal.getText().isEmpty() || !txtDataVal.getText().matches("[0-9][0-9][/][0-9][0-9][/][0-9][0-9][0-9][0-9]"))){
				FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/GUI/PopUpTela.fxml"));
	            Parent root1 = (Parent) fxmlLoader.load();
	            Stage stage = new Stage();
	            stage.initModality(Modality.APPLICATION_MODAL);
