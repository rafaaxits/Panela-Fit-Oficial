...
public class VendaPaneController {
	...
	public void cadastrarVenda() throws ValidationException, IOException, ClienteNaoExisteException,
			FuncionarioNaoExisteException, VendaJaExisteException, VendaNaoExisteException {
		if (validateFields()) {
			try {
	...
	public void alterarVenda() throws VendaNaoExisteException, FormatacaoInvalidaException, ValidationException,
			IOException, ClienteNaoExisteException, FuncionarioNaoExisteException {
		if (validateFields()) {
			try {
				Integer codigoCliente = new Integer(txtCodigoCliente.getText());
				Integer codigoFuncionario = new Integer(txtCodigoFuncionario.getText());
	...
	private void initialize() {
		...
		colunaEnderecoCliente
				.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getEndereco()));
		colunaTelefoneCliente
				.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getTelefone()));
		...
		colunaEnderecoFuncionario
				.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getEndereco()));
		colunaTelefoneFuncionario
				.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getTelefone()));
		...
		colunaClienteCpf
				.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getCliente().getCpf()));
		colunaFuncionario.setCellValueFactory(
				cellData -> new SimpleStringProperty(cellData.getValue().getFuncionario().getNome()));
		colunaFuncionarioCpf.setCellValueFactory(
				cellData -> new SimpleStringProperty(cellData.getValue().getFuncionario().getCpf()));
		colunaListaItens.setCellValueFactory(
				cellData -> new SimpleStringProperty(cellData.getValue().getListaItensDeVenda().toString()));
		colunaDataVenda.setCellValueFactory(
				cellData -> new SimpleStringProperty(cellData.getValue().getDataDaVenda().toString()));
		colunaTotal
				.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getTotal().toString()));
		...
		/*
		 * ArrayList<Venda> alteradas = new ArrayList<Venda>(); for(Venda v:
		 * this.panelaFit.listarVendas()){ v.setTotal(v.calcularVenda());
		 * alteradas.add(v); }
		 * 
		 * alteradas.forEach(e->{ try{ this.panelaFit.alterarVenda(e);
	...
	public boolean validateFields() throws IOException {
		...
		try {
			if ((txtCodigoVenda.getText().isEmpty() || !txtCodigoVenda.getText().matches("[0-9][0-9][0-9][0-9][0-9]"))
					|| (txtDataVenda.getText().isEmpty()
							|| !txtDataVenda.getText().matches("[0-9][0-9][/][0-9][0-9][/][0-9][0-9][0-9][0-9]"))) {
				FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/GUI/view/PopUpTela.fxml"));
				Parent root1 = (Parent) fxmlLoader.load();
				Stage stage = new Stage();
				stage.initModality(Modality.APPLICATION_MODAL);
	...
	public void setItemNaLista() throws IOException, ProdutoNaoExisteException {
		if (validateListaItem()) {
			...
			colunaNomeProdutoItemVenda.setCellValueFactory(
					cellData -> new SimpleStringProperty(cellData.getValue().getProduto().getNome()));
