...
public class ClientePaneController {
	...
	public void alterarCliente()
			throws ClienteNaoExisteException, FormatacaoInvalidaException, ValidationException, IOException {
	...
	private boolean validateFields() throws IOException {
		...
		try {
			if ((txtNomeCliente.getText().isEmpty() || !txtNomeCliente.getText().matches("[a-z A-Z]+"))
					|| (txtCpfCliente.getText().isEmpty() || !txtCpfCliente.getText()
							.matches("[0-9][0-9][0-9].[0-9][0-9][0-9].[0-9][0-9][0-9]-[0-9][0-9]"))
					|| (txtTelefoneCliente.getText().isEmpty() || !txtTelefoneCliente.getText()
							.matches("[(][0-9][0-9][)][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]"))
					|| (txtIdadeCliente.getText().isEmpty() || !txtIdadeCliente.getText().matches("[0-9][0-9]"))
					|| txtEnderecoCliente.getText().isEmpty() || (txtCodigoCliente.getText().isEmpty()
							|| !txtCodigoCliente.getText().matches("[0-9][0-9][0-9][0-9][0-9]"))) {
				FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/GUI/view/PopUpTela.fxml"));
				Parent root1 = (Parent) fxmlLoader.load();
				Stage stage = new Stage();
				stage.initModality(Modality.APPLICATION_MODAL);
