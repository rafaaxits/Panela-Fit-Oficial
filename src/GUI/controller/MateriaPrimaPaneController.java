...
public class MateriaPrimaPaneController {
		...
		if (validateFields()) {
			try {
				...
				Double preco = new Double(
						txtPrecoMateriaPrima.getText().replace("R", "").replace("$", "").replace(".", ""));
	...
	public void alterarMateriaPrima()
			throws MateriaPrimaNaoExisteException, ValidationException, IOException, FormatacaoInvalidaException {
	...
	private boolean validateFields() throws IOException {
		...
		try {
			if ((txtNomeMateriaPrima.getText().isEmpty() || !txtNomeMateriaPrima.getText().matches("[a-z A-Z]+"))
					|| (txtQuantidadeMateriaPrima.getText().isEmpty()
							|| !txtQuantidadeMateriaPrima.getText().matches("[0-9][0-9]KG"))
					|| (txtPrecoMateriaPrima.getText().isEmpty()
							|| !txtPrecoMateriaPrima.getText().matches("[R][$][0-9][0-9][0-9]"))
					|| (txtCodigoMateriaPrima.getText().isEmpty()
							|| !txtCodigoMateriaPrima.getText().matches("[0-9][0-9][0-9][0-9][0-9]"))) {
				FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/GUI/view/PopUpTela.fxml"));
				Parent root1 = (Parent) fxmlLoader.load();
				Stage stage = new Stage();
				stage.initModality(Modality.APPLICATION_MODAL);
