...
public class TesteFachada {
	...
	public static void main(String[] args)  {
		...
		try{
			panelaFit.cadastrarFuncionario(f1);//tentando cadastrar um funcionario que j√° esta cadastrado e a Exception apareceu
		}catch(Exception e){
			e.printStackTrace();
			System.out.print("\n");
		}
		...
		try{
			panelaFit.alterarFuncionario(f2,f5);//Altera√ß√£o ok, Alterando um func cadastrado por outro que ainda nao estava cadastrado, ap√≥s A ALTERACAO O NOVO FUNC FOI CADASTRADO, TUDO OK
		}catch(Exception e){
			e.printStackTrace();
			System.out.println("\n");
		}
		...
		try{
			panelaFit.removerFuncionario(f2);//tentando remover um funcionario que j√° foi removido e que nao existe mais no sistema
		}catch(Exception e){
			e.printStackTrace();
			System.out.println("\n");
		}
...
//FORNECEDORES
		...
		Fornecedor for3 = new Fornecedor("EspaÁoSaldavel", "Av bernardo vieira", "33444123", 133);
